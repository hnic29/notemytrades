"use client";

import {
  PolarAngleAxis,
  PolarGrid,
  Radar,
  RadarChart,
  ResponsiveContainer,
} from "recharts";
import { cn } from "@/lib/utils";
import type { TradeScore } from "@/lib/analytics/stats";

function toneFor(score: number) {
  return score >= 70 ? "text-profit" : score >= 40 ? "text-warning" : "text-loss";
}

export function TradeScoreGauge({ score }: { score: TradeScore | null }) {
  if (score == null) {
    return (
      <div className="flex h-full flex-col items-center justify-center gap-1 py-8 text-center">
        <div className="text-3xl font-semibold text-text-faint">—</div>
        <div className="text-xs text-text-faint">Log 5+ closed trades to unlock</div>
      </div>
    );
  }

  const tone = toneFor(score.overall);
  const circumference = 2 * Math.PI * 42;
  const offset = circumference * (1 - score.overall / 100);
  const radarData = score.factors.map((f) => ({ label: f.label, score: f.score }));

  return (
    <div className="flex flex-col items-center gap-4 sm:flex-row sm:items-center sm:justify-around">
      <div className="flex flex-col items-center gap-1">
        <div className="relative h-[104px] w-[104px]">
          <svg width="104" height="104" viewBox="0 0 104 104" className="-rotate-90">
            <circle cx="52" cy="52" r="42" stroke="#232c40" strokeWidth="10" fill="none" />
            <circle
              cx="52"
              cy="52"
              r="42"
              strokeWidth="10"
              fill="none"
              strokeLinecap="round"
              strokeDasharray={circumference}
              strokeDashoffset={offset}
              className={cn(tone, "stroke-current transition-all duration-500")}
            />
          </svg>
          <div className={cn("absolute inset-0 flex items-center justify-center text-2xl font-semibold", tone)}>
            {score.overall}
          </div>
        </div>
        <div className="text-xs text-text-faint">Trade Score</div>
      </div>

      <div className="h-56 w-full max-w-xs">
        <ResponsiveContainer width="100%" height="100%">
          <RadarChart data={radarData} outerRadius="70%">
            <PolarGrid stroke="#232c40" />
            <PolarAngleAxis dataKey="label" tick={{ fill: "#93a0b8", fontSize: 10 }} />
            <Radar dataKey="score" stroke="#2dd4bf" fill="#2dd4bf" fillOpacity={0.28} strokeWidth={2} />
          </RadarChart>
        </ResponsiveContainer>
      </div>

      <ul className="grid w-full grid-cols-2 gap-x-4 gap-y-1.5 text-xs sm:w-40 sm:grid-cols-1">
        {score.factors.map((f) => (
          <li key={f.key} className="flex items-center justify-between gap-2">
            <span className="text-text-faint">{f.label}</span>
            <span className={cn("font-medium", toneFor(f.score))}>{f.score}</span>
          </li>
        ))}
      </ul>
    </div>
  );
}
